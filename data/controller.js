...
function mountModuleSettings(){
  ...
  ['pwrap','ywrap','usageWrap','mwrap','swrap','termPanel','cwrap'].forEach(id=>{
...
async function saveWeatherSuggestion(item){
  let city=item.name||'LOCATION';
  try{
    const r=await fetch('https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat='+encodeURIComponent(item.latitude)+'&lon='+encodeURIComponent(item.longitude)+'&zoom=10&accept-language=en',{headers:{Accept:'application/json'}});
    const j=await r.json(),a=j.address||{};
    city=a.city||a.town||a.municipality||a.county||city;
  }catch(e){}
  await saveWeatherLocation(item.latitude,item.longitude,city,'manual');
}
function renderWeatherSuggestions(results){const out=document.getElementById('weatherSuggestions');out.innerHTML='';results.forEach(item=>{const button=document.createElement('button');button.type='button';button.className='weather-suggestion';const name=document.createElement('span');name.textContent=item.name||'LOCATION';const detail=document.createElement('small');detail.textContent=[item.admin1,item.country].filter(Boolean).join(' · ');button.append(name,detail);button.onclick=()=>saveWeatherSuggestion(item);out.appendChild(button);});}
...
function closeModuleSettings(){
  ['pwrap','ywrap','usageWrap','mwrap','swrap','uwrap','weatherLocationWrap','termPanel','cwrap'].forEach(id=>document.getElementById(id)?.classList.remove('open'));
...
async function setView(v){if(isBusy||termOpen||canvasOpen)return;if(activeView===19)await leaveMediaForView();if(v===3){toggleCanvas();return;}const keys={0:'w',1:'s',2:'d',6:'c',7:'p',8:'e',9:'m',10:'k',17:'y',18:'u',20:'q'};if(!keys[v]||!await req('/cmd?k='+keys[v]))return;closeModuleSettings();activeView=v;document.querySelectorAll('.vbtn').forEach(b=>b.classList.toggle('active',parseInt(b.dataset.v)===v));const panels={7:'pwrap',17:'ywrap',9:'mwrap',10:'swrap',18:'uwrap',20:'usageWrap'};Object.entries(panels).forEach(([view,id])=>document.getElementById(id)?.classList.toggle('open',Number(view)===v));document.getElementById('weatherLocationWrap').classList.toggle('open',v===8);document.getElementById('termPanel').classList.toggle('open',v===2);if(v===20){await loadUsage();clearInterval(usagePollTimer);usagePollTimer=setInterval(loadUsage,15000);toast('usage open');return;}if(v===18){await Promise.all([loadTimetable(),pollTimetableStatus()]);toast('timetable open');return;}if(v===17){await loadSalaryConfig();await pollSalary();toast('live ledger open');return;}if(v===9){await loadMarketConfig();loadMarketDirectory();toast('crypto open');return;}if(v===10){await loadStockConfig();toast('market open');return;}if(v===2){termOpen=true;document.getElementById('twrap').classList.add('open');setBusy(false);setBusy(false);document.querySelectorAll('.vbtn,.lbtn').forEach(b=>b.disabled=true);document.getElementById('tin').focus();toast('terminal open');return;}if(v===6||v===7||v===8){toast(v===6?'clock open':(v===7?'pomodoro open':'locating weather'));return;}setBusy(true);await waitNotBusy();setBusy(false);}
function formatUsageReset(minutes){if(!Number.isFinite(minutes)||minutes<0)return 'RESET --';const days=Math.floor(minutes/1440),hours=Math.floor((minutes%1440)/60),mins=minutes%60;return 'RESET '+(days?days+'D ':'')+(hours?hours+'H ':'')+mins+'M';}
function renderUsage(j){usageState=j;const state=document.getElementById('usageState'),status=document.getElementById('usageStatus');if(!state||!status)return;state.textContent=j.authError?'TOKEN ERROR':(j.loading?'LOADING':(j.valid?'LIVE':(j.configured?'READY':'NOT SET')));status.textContent=j.authError?'Credential rejected. Replace it and refresh.':j.loading?'Mochi is querying Anthropic directly...':j.valid?'Quota is read on the device; browser only configures the credential.':j.configured?'Credential saved. Press refresh to query usage.':'Add a Claude credential to let the device query its own quota.';const rows=[['session',j.sessionLeft,j.sessionResetMins],['weekly',j.weeklyLeft,j.weeklyResetMins]];rows.forEach(([key,left,reset])=>{const value=Number(left),pct=Number.isFinite(value)&&value>=0?Math.min(100,Math.max(0,value)):0;document.getElementById('usage'+(key==='session'?'5h':'7d')+'Left').textContent=Number.isFinite(value)&&value>=0?value.toFixed(1)+'% LEFT':'--% LEFT';document.getElementById('usage'+(key==='session'?'5h':'7d')+'Bar').style.width=pct+'%';document.getElementById('usage'+(key==='session'?'5h':'7d')+'Reset').textContent=formatUsageReset(Number(reset));});}
async function loadUsage(){try{const r=await fetch('/usage/config',{cache:'no-store'});if(!r.ok)throw new Error();renderUsage(await r.json());}catch(e){const status=document.getElementById('usageStatus');if(status)status.textContent='Usage status unavailable.';}}
async function saveUsageToken(){const input=document.getElementById('usageToken'),token=input.value.trim();if(!token){toast('enter a credential',false);return;}try{const r=await fetch('/usage/config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token})});const j=await r.json();if(!r.ok)throw new Error(j.error||'credential rejected');input.value='';renderUsage(j);toast('credential saved');}catch(e){toast(e.message||'credential save failed',false);}}
async function clearUsageToken(){try{const r=await fetch('/usage/config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({clear:true})});if(!r.ok)throw new Error();renderUsage(await r.json());toast('credential cleared');}catch(e){toast('credential clear failed',false);}}
async function refreshUsage(){try{const r=await fetch('/usage/refresh',{method:'POST'});if(!r.ok)throw new Error();await loadUsage();toast('usage refresh requested');}catch(e){toast('usage refresh failed',false);}}
...
let carouselPages=[{id:6,name:'Clock'},{id:7,name:'Pomodoro'},{id:8,name:'Weather'},{id:9,name:'Crypto'},{id:10,name:'Market'},{id:17,name:'Live Ledger'},{id:18,name:'Timetable'},{id:20,name:'Usage'}];
