...
function wifiStatusHtml(j){const ap='http://'+(j.apIp||'192.168.4.1');const mdns=j.mdns||'http://clawd-mochi.local';const line='font-size:10px;color:#8a8278;margin-top:3px';if(j.connected){const lan='http://'+(j.lanIp||j.ip);return '<span style="color:#28b878">connected: '+j.ssid+'</span><div style="'+line+'">LAN: '+lan+'</div><div style="'+line+'">Name: '+mdns+'</div><div style="'+line+'">AP: '+ap+' ('+(j.apSsid||'ClaWD-Mochi')+')</div>';}if(j.configured&&j.savedSsid){const failed=j.retryCount>0||j.retryExhausted;const state=failed?(j.lastError||'Connection failed'):(j.phase||'Connecting');const retry=j.retryCount>0?' - retry '+j.retryCount:'';return '<span style="color:#c96a3e">'+state+retry+'</span><div style="'+line+'">saved: '+j.savedSsid+'</div><div style="'+line+'">AP still works: '+ap+'</div>';}return 'not connected - scan to setup<div style="'+line+'">AP: '+ap+' ('+(j.apSsid||'ClaWD-Mochi')+')</div>';}
async function connectWifi(){const ssid=document.getElementById('wssid').value.trim();const pass=document.getElementById('wpass').value;if(!ssid){toast('enter SSID',false);return;}const fd=new FormData();fd.append('ssid',ssid);fd.append('password',pass);document.getElementById('wstatus').textContent='connecting...';try{await fetch('/wifi/connect',{method:'POST',body:fd});}catch(e){}let ok=false,last=null;for(let i=0;i<30;i++){await new Promise(r=>setTimeout(r,1000));try{const r=await fetch('/wifi/status');last=await r.json();if(last.connected){ok=true;break;}document.getElementById('wstatus').innerHTML=wifiStatusHtml(last);}catch(e){}}if(ok){await pollWifiStatus();document.getElementById('wlist').style.display='none';document.getElementById('wform').style.display='none';toast('wifi connected');}else{if(last)document.getElementById('wstatus').innerHTML=wifiStatusHtml(last);toast((last&&last.lastError)||'wifi failed',false);}}
