...
| `DisplayService` | View rendering — eyes, Claude Code panel, clock, pomodoro, canvas, terminal |
...
`DisplayService` has two top-level modes: `EXPRESSION` (eyes) and `INFO` (Claude Code panel). When in `INTERACTIVE` mode (triggered by web controller), it can show: normal eyes, squish eyes, code view, canvas draw, thinking, working, clock, or pomodoro.

### Claude Code Hook

`scripts/cc_hook.py` is a Python script installed into Claude Code's settings as a hook. It sends session events to Clawd Mochi over serial (preferred) or UDP (fallback). Install with `scripts/install_claude_hook.sh`.
